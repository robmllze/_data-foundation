...
final class IdUtils {
  ...
  static String mapPubIdToUserId({required String userPubId}) {
    return _unmapString1(userPubId.substring("$ACCESS_ID_PREFIX-".length)).substring(3);
  }
}
...
String _mapString1(String input) {
  return String.fromCharCodes(input.codeUnits.map((c) => _mapChar1(c, input.length)));
}
...
String _unmapString1(String input) {
  return String.fromCharCodes(input.codeUnits.map((c) => _unmapChar1(c, input.length)));
}
